...
public class ProcessManifest {
    ...
    protected Set<String> exportedComponents = new HashSet<>();


    public Map<String, List<IntentFilter>> getFilters() {
        return filters;
    }
    ...
    protected List<String> receiverNames = new ArrayList<>();
    protected List<String> serviceNames = new ArrayList<>();

    protected Map<String, List<IntentFilter>> filters = new HashMap<>();
    protected Map<String, ComponentType> compTypes = new HashMap<>();
	/**
	...
	protected void handle(InputStream manifestIS) throws IOException, XmlPullParserException {
        ...
        //collect intent-filters and name mappings
        for(AXmlNode node: this.services)
        {
            String componentName = expandClassName((String) node.getAttribute("name").getValue());
            this.serviceNames.add(componentName);
            collectIntentFilters(node);
            if(isComponentExported(node))
            {
                exportedComponents.add(componentName);
            }
            compTypes.put(componentName, ComponentType.Service);
        }
        for(AXmlNode node: this.activities)
        {
            String componentName = expandClassName((String) node.getAttribute("name").getValue());
            this.activityNames.add(componentName);
            collectIntentFilters(node);
            if(isComponentExported(node))
            {
                exportedComponents.add(componentName);
            }
            compTypes.put(componentName, ComponentType.Activity);
        }
        for(AXmlNode node: this.receivers)
        {
            String componentName = expandClassName((String) node.getAttribute("name").getValue());
            this.receiverNames.add(componentName);
            collectIntentFilters(node);
            if(isComponentExported(node))
            {
                exportedComponents.add(componentName);
            }
            compTypes.put(componentName, ComponentType.BroadcastReceiver);
        }
	}
	...
	public Set<String> getAccessibleComponents()
	{
		Set<String> entryPoints = new HashSet<>();
		for (AXmlNode node : this.activities)
			checkPublicAccessibleCompoenent(entryPoints, node);
	...
	public Set<String> getEntryPointClasses() {
		...
		for (AXmlNode node : this.activities)
			if (JadeCfg.public_component_only_enabled()) {
				checkPublicAccessibleCompoenent(entryPoints, node);
			}
			else
			{
		...
		//TODO: add dynamic broadcast receivers
    ...
    public ComponentType getComponentTypeByFullName(String className) {
        return compTypes.get(className);
    }
	/**
